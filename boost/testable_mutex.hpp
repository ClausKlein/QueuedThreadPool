 ...
 *
 * Make our mutex testable if it isn't already locked by this thread.
 *
...
template <typename Lockable> class testable_mutex {
    Lockable mtx_;
...
template <typename Lockable> struct is_testable_lockable : false_type {
};
...
//  /**
//   * Overloaded function used to check if the mutex is locked when it is
//   testable and do nothing otherwise.
//   *
//   * This function is used usually to assert the pre-condition when the
//   function can only be called when the mutex
