...
//  http://www.boost.org/libs/libs/chrono/stopwatches for documentation.
...
namespace chrono
{
    ...
    /**
     * A laps_stopwatch is a model of @c Stopwatch taking as template
     * parameters the @c Clock and the @c LapsCollector.
     *
     * The main difference respect to a @c simple_stopwatch is that the user
     * can stop it. Each sequence of start-stop results in a new elapsed
     * duration sample that is provided to the LapsCollector.
     *
     * It is up to the LapsCollector to make whatever wants with each sample.
     * A LapCollector must define a store(duration const&) and a clear()
     * functions.
     *
     * The library provides LapsCollectors that forget the sample, store the
     * last one, cummulates the samples in an accumulator set or store them in
     * a container. For simplicity the default LapCollector is the one that
     * forget the samples.
     *
    ...
    template <typename Clock   = high_resolution_clock,
        typename LapsCollector = no_memory<typename Clock::duration> >
    ...
    public:
         ...
         * Default constructor.
         *
         * Effects: Assign the error code if any internal error occur while
         * retrieving the current time. Effects: Starts the stopwatch.
         * Post-conditions: is_running() if no error occur.
         */
        explicit laps_stopwatch(system::error_code& ec)
            : start_(duration::zero())
            , running_(false)
            , laps_collector_()
        {
         ...
         * Post-conditions: ! is_running() if no error occur.
         */
        explicit laps_stopwatch(const dont_start_t&)
            : start_(duration::zero())
            , running_(false)
            , laps_collector_()
        { }
         ...
         */
        explicit laps_stopwatch(laps_collector const& acc)
            : start_(duration::zero())
            , running_(false)
            , laps_collector_(acc)
        {
         ...
         *
         * Effects: Assign the error code if any internal error occur while
         * retrieving the current time. Effects: Copies the LapsCollector.
         * Starts the stopwatch. Post-conditions: is_running() if no error
         * occur.
         *
         * Remark: The LapsCollector is copied and owned by the stopwatch.
         */
        explicit laps_stopwatch(
            laps_collector const& acc, system::error_code& ec)
            : start_(duration::zero())
            , running_(false)
            , laps_collector_(acc)
        {
         ...
         * Remark: The LapsCollector is copied and owned by the stopwatch.
         */
        laps_stopwatch(laps_collector const& acc, const dont_start_t&)
            : start_(duration::zero())
            , running_(false)
            , laps_collector_(acc)
        { }
         ...
         *
         * Effects: Assign the error code if any internal error occur while
         * retrieving the current time. Effects: As if stop(); start() were
         * called, but ensuring that the start time is the same as the stop
         * time.
         *
         * Post-conditions: is_running() if no error occur.
         */
        ...
        void restart(system::error_code& ec)
        {
            ...
            if (!BOOST_CHRONO_IS_THROWS(ec) && ec)
                return;
         ...
         *
         * Effects: Assign the error code if any internal error occur while
         * retrieving the current time. Effects: Memorize the current time.
         *
         * Post-conditions: is_running() if no error occur.
         */
         ...
         * Start the stopwatch.
         *
         * Effects: Gives the elapsed time since start time to the
         * LapCollector.
         *
         ...
         *
         * Effects: Assign the error code if any internal error occur while
         * retrieving the current time. Effects: Gives the elapsed time since
         * start time to the LapCollector if no internal error occurs.
         *
         ...
         *
         * Returns: the elapsed time since the last start if no internal error
         * occur.
         *
         */
         ...
         *
         * Returns: the elapsed time since the start if no internal error
         * occur.
         *
         */
        ...
        duration last() const { return laps_collector_.last(); }
        /**
