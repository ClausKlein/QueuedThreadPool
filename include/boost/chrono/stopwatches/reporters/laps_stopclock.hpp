...
#    if !defined(BOOST_ENABLE_WARNINGS) \
        && !defined(BOOST_CHRONO_ENABLE_WARNINGS)
#        if defined __GNUC__
...
namespace chrono
{
    ...
    template <class CharT, typename Clock, typename LapsCollector,
        class Formatter>
    class basic_stopclock : public basic_stopwatch_reporter<CharT,
                                stopwatch<Clock, LapsCollector>, Formatter> {
    public:
        typedef basic_stopwatch_reporter<CharT,
            stopwatch<Clock, LapsCollector>, Formatter>
            base_type;
        typedef Clock clock;
...
#    if !defined BOOST_CHRONO_DONT_PROVIDE_HYBRID_ERROR_HANDLING
        ...
        { }
#    endif
        explicit basic_stopclock(const dont_start_t& tag) BOOST_NOEXCEPT
            : base_type(tag)
        { }
        ...
        explicit basic_stopclock(typename Formatter::string_type const& fmt)
            : base_type(fmt)
        { }
        typedef stopwatch_runner<
            basic_stopclock<CharT, Clock, LapsCollector, Formatter> >
            scoped_run;
        typedef stopwatch_stopper<
            basic_stopclock<CharT, Clock, LapsCollector, Formatter> >
            scoped_stop;
        typedef stopwatch_suspender<
            basic_stopclock<CharT, Clock, LapsCollector, Formatter> >
            scoped_suspend;
        typedef stopwatch_resumer<
            basic_stopclock<CharT, Clock, LapsCollector, Formatter> >
            scoped_resume;
    ...
    template <typename Clock   = high_resolution_clock,
        ...
        class Formatter = typename basic_stopwatch_reporter_default_formatter<
            char, stopwatch<Clock, LapsCollector> >::type>
    class stopclock;

    template <class Stopwatch, class Formatter>
    struct basic_stopwatch_reporter_default_formatter<char,
        stopclock<Stopwatch, Formatter> > {
        typedef Formatter type;
    };

    template <typename Clock, typename LapsCollector, class Formatter>
    class stopclock
        : public basic_stopclock<char, Clock, LapsCollector, Formatter> {
        typedef basic_stopclock<char, Clock, LapsCollector, Formatter>
            base_type;
...
#    if !defined BOOST_CHRONO_DONT_PROVIDE_HYBRID_ERROR_HANDLING
        ...
        { }
#    endif
        explicit stopclock(const dont_start_t& tag) BOOST_NOEXCEPT
            : base_type(tag)
        { }
        ...
        typedef stopwatch_stopper<stopclock<Clock, LapsCollector, Formatter> >
            scoped_stop;
        typedef stopwatch_suspender<
            stopclock<Clock, LapsCollector, Formatter> >
            scoped_suspend;
