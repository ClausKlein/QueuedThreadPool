...
#define BOOST_CHRONO_STOPWATCHES_TIMES_FORMAT_DEFAULT \
            ...
            this->os_ << static_cast<format_type&>(*this)
                    % io::group(std::fixed,
                        ...
                        boost::chrono::duration<double, Ratio>(
                            nanoseconds(times.real)))
                    % io::group(std::fixed,
                        std::setprecision(this->precision_),
                        duration_fmt(this->duration_style_),
                        boost::chrono::duration<double, Ratio>(
                            nanoseconds(times.user)))
                    % io::group(std::fixed,
                        std::setprecision(this->precision_),
                        duration_fmt(this->duration_style_),
                        boost::chrono::duration<double, Ratio>(
                            nanoseconds(times.system)))
                    % io::group(std::fixed,
                        std::setprecision(this->precision_),
                        duration_fmt(this->duration_style_),
                        boost::chrono::duration<double, Ratio>(
                            nanoseconds(times.user + times.system)))
...
#    define BOOST_CHRONO_STOPWATCHES_TIMES_FORMAT(F) \
        boost::chrono::detail::adaptive_string( \
            F " : " BOOST_CHRONO_STOPWATCHES_TIMES_FORMAT_DEFAULT)
#    ifdef __GNUC__
