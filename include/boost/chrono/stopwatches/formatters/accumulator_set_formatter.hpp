...
//  ------------------------------------------------------------// Copyright
//  2011 Vicente J. Botet Escriba Distributed under the Boost Software License,
//  Version 1.0. See http://www.boost.org/LICENSE_1_0.txt See
...
#define BOOST_CHRONO_STOPWATCHES_ACCUMULATOR_SET_FORMAT_DEFAULT \
...
namespace chrono
{
    ...
    class basic_accumulator_set_formatter
        : public base_formatter<CharT, Traits>,
          public basic_format<CharT, Traits> {
    ...
    public:
        ...
        basic_accumulator_set_formatter()
            : base_type()
            , format_type(
                  BOOST_CHRONO_STOPWATCHES_ACCUMULATOR_SET_FORMAT_DEFAULT)
        { }
        ...
        { }
        basic_accumulator_set_formatter(
            const char* fmt, ostream_type& os = std::cout)
            : base_type(os)
            , format_type(fmt)
        { }
        basic_accumulator_set_formatter(
            string_type const& fmt, ostream_type& os = std::cout)
            : base_type(os)
            , format_type(fmt)
        { }
        ...
        )
        {
            ...
            this->os_ << static_cast<format_type&>(*this)
                    ...
                    % io::group(std::fixed,
                        std::setprecision(this->precision_),
                        duration_fmt(this->duration_style_),
                        (boost::accumulators::count(acc) == 0)
                            ? boost::chrono::duration<double, Ratio>(
                                duration_t::zero())
                            : boost::chrono::duration<double, Ratio>(
                                duration_t(boost::accumulators::sum(acc))))
                    % io::group(std::fixed,
                        std::setprecision(this->precision_),
                        duration_fmt(this->duration_style_),
                        (boost::accumulators::count(acc) == 0)
                            ? boost::chrono::duration<double, Ratio>(
                                duration_t::zero())
                            : boost::chrono::duration<double, Ratio>(
                                duration_t((boost::accumulators::min)(acc))))
                    % io::group(std::fixed,
                        std::setprecision(this->precision_),
                        duration_fmt(this->duration_style_),
                        (boost::accumulators::count(acc) == 0)
                            ? boost::chrono::duration<double, Ratio>(
                                duration_t::zero())
                            : boost::chrono::duration<double, Ratio>(
                                duration_t((boost::accumulators::max)(acc))))
                    % io::group(std::fixed,
                        std::setprecision(this->precision_),
                        duration_fmt(this->duration_style_),
                        ((boost::accumulators::count(acc) > 0)
...
#    define BOOST_CHRONO_STOPWATCHES_ACCUMULATOR_SET_FORMAT(F) \
        boost::chrono::detail::adaptive_string( \
            F " : " BOOST_CHRONO_STOPWATCHES_ACCUMULATOR_SET_FORMAT_DEFAULT)
#    ifdef __GNUC__
...
#    else
#        define BOOST_CHRONO_STOPWATCHES_ACCUMULATOR_SET_FUNCTION_FORMAT \
            BOOST_CHRONO_STOPWATCHES_ACCUMULATOR_SET_FORMAT( \
