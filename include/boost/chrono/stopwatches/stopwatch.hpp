...
namespace chrono
{
    ...
    /**
     * A stopwatch is a model of @c Stopwatch taking as parameters the @c Clock
     * and the @c LapsCollector.
     *
    ...
    public:
         ...
         * Post-conditions: ! is_running() if no error occur.
         */
        explicit stopwatch(const dont_start_t&)
            : start_(duration::zero())
            , running_(false)
            , laps_collector_()
        { }
         ...
         */
        explicit stopwatch(laps_collector const& acc)
            : start_(duration::zero())
            , running_(false)
            , laps_collector_(acc)
        {
         ...
         *
         * Requires: is_running().
         * Effects: Stores the elapsed time since start time into the
         * LapCollector.
         *
         ...
         * Requires: is_running().
         * Effects: Assign the error code if any internal error occur while
         * retrieving the current time. Effects: Stores the elapsed time since
         * start time into the LapCollector if no internal error occurs.
         *
         * Throws: Any exception that the LapCollector can Throw.
         *
         * Post-conditions: !is_running() if no error occur.
