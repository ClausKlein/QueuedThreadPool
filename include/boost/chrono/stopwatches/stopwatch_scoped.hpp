...
//  ------------------------------------------------------------// Copyright
//  2009-2011 Vicente J. Botet Escriba Distributed under the Boost Software
...
namespace chrono
{
...
#endif
        ~stopwatch_runner() { stopwatch_.stop(); }
...
#endif
        ~stopwatch_stopper() { stopwatch_.start(); }
    ...
    public:
        ...
        ~stopwatch_suspender() { stopwatch_.resume(); }

    private:
        stopwatch& stopwatch_;
        stopwatch_suspender();                           // = delete;
        stopwatch_suspender(const stopwatch_suspender&); // = delete;
        stopwatch_suspender& operator=(
            const stopwatch_suspender&); // = delete;
    };
...
#endif
        ~stopwatch_resumer() { stopwatch_.suspend(); }
