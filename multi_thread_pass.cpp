...
template <typename ValueType> struct call_push {
...
template <typename ValueType> struct call_pull {
    boost::sync_queue<ValueType>* q_;
    boost::barrier* go_;
...
template <typename ValueType> struct call_wait_pull {
    boost::sync_queue<ValueType>* q_;
    boost::barrier* go_;
...
void test_concurrent_push_and_pull_on_empty_queue()
{
    ...
    boost::future<ValueType> pull_done;

    try {
        push_done =
            boost::async(boost::launch::async, call_push<ValueType>(&q, &go));
        pull_done =
            boost::async(boost::launch::async, call_pull<ValueType>(&q, &go));

        push_done.get();
        BOOST_TEST_EQ(pull_done.get(), 42U);
        BOOST_TEST(q.empty());
    } catch (...) {
        BOOST_TEST(false);
    }
    ...
    try {
        for (unsigned int i = 0; i < n; ++i) {
            pull_done[i] = boost::async(boost::launch::async,
                call_wait_pull<ValueType>(&q, &go), boost::ref(results[i]));
        }
        ...
        for (unsigned int i = 0; i < n; ++i) {
            ...
            // FIXME BOOST_TEST_EQ(results[i], 42 + i);
            BOOST_TEST_NE(results[i], 41U);
        }
        BOOST_TEST(q.empty());
    } catch (...) {
        BOOST_TEST(false);
...
void test_concurrent_push_on_empty_queue()
{
    ...
    try {
        for (unsigned int i = 0; i < n; ++i) {
            push_done[i] = boost::async(
                boost::launch::async, call_push<ValueType>(&q, &go));
        }
    ...
    try {
        BOOST_TEST(!q.empty());
        for (unsigned int i = 0; i < n; ++i) {
            // FIXME BOOST_TEST_EQ(q.pull(), 42 + i);
            BOOST_TEST_NE(q.pull(), 41U);
        }
        BOOST_TEST(q.empty());
...
void test_concurrent_pull_on_queue()
{
    ...
    boost::future<ValueType> pull_done[n];
    ...
    try {
        ...
        for (unsigned int i = 0; i < n; ++i) {
            // FIXME BOOST_TEST_EQ(pull_done[i].get(), 42 + i);
            BOOST_TEST_NE(pull_done[i].get(), 41U);
        }
        BOOST_TEST(q.empty());
    } catch (...) {
        BOOST_TEST(false);
